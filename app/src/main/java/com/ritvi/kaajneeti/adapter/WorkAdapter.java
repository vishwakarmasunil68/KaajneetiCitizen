...
public class WorkAdapter extends RecyclerView.Adapter<WorkAdapter.ViewHolder>{
    private List<WorkPOJO> items;
    Activity activity;
    Fragment fragment;

    public WorkAdapter(Activity activity, Fragment fragment, List<WorkPOJO> items) {
        this.items = items;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.tv_title.setText(items.get(position).getWorkCompany());

        String date=items.get(position).getWorkFrom();
        if(items.get(position).getCurrentlyWorking().equals("1")){
            date+=" - Present";
        }else{
            date+=" - "+items.get(position).getWorkTo();
        }

        holder.tv_date.setText(date);
        holder.tv_position.setText(items.get(position).getWorkPosition());
                ...
                builder.setTitle("Delete")
                        ...
                        .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                nameValuePairs.add(new BasicNameValuePair("work_id", items.get(position).getUserProfileWorkId()));
        ...
        holder.ll_edit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (activity instanceof HomeActivity) {
                    ...
                    bundle.putSerializable("workPOJO", items.get(position));
